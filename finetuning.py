# coding=utf-8
# Base on: https://github.com/sunlab-osu/TURL/blob/release_ongoing/run_table_CT_finetuning.py
...
def train(args, config, train_dataset, model, eval_dataset = None):
    ...
    train_dataloader = WikiDataLoader(train_dataset, sampler=train_sampler, batch_size=args.train_batch_size, is_train=True)
    ...
    optimizer_grouped_parameters = [
        ...
        {'params': [p for n, p in model.cls1.named_parameters() if not any(nd in n for nd in no_decay)], 'weight_decay': args.weight_decay, 'lr': args.learning_rate*10 if args.cls_learning_rate==0 else args.cls_learning_rate},
        {'params': [p for n, p in model.cls1.named_parameters() if any(nd in n for nd in no_decay)], 'weight_decay': 0.0, 'lr': args.learning_rate*10 if args.cls_learning_rate==0 else args.cls_learning_rate},
        {'params': [p for n, p in model.cls2.named_parameters() if not any(nd in n for nd in no_decay)], 'weight_decay': args.weight_decay, 'lr': args.learning_rate*10 if args.cls_learning_rate==0 else args.cls_learning_rate},
        {'params': [p for n, p in model.cls2.named_parameters() if any(nd in n for nd in no_decay)], 'weight_decay': 0.0, 'lr': args.learning_rate*10 if args.cls_learning_rate==0 else args.cls_learning_rate},
    ]
    if hasattr(model, "fc1"):
        optimizer_grouped_parameters.extend([
            {'params': [p for n, p in model.fc1.named_parameters() if not any(nd in n for nd in no_decay)], 'weight_decay': args.weight_decay, 'lr': args.learning_rate * 10 if args.cls_learning_rate == 0 else args.cls_learning_rate},
            {'params': [p for n, p in model.fc1.named_parameters() if any(nd in n for nd in no_decay)], 'weight_decay': 0.0, 'lr': args.learning_rate * 10 if args.cls_learning_rate == 0 else args.cls_learning_rate},
        ])
    if hasattr(model, "fc2"):
        optimizer_grouped_parameters.extend([
            {'params': [p for n, p in model.fc2.named_parameters() if not any(nd in n for nd in no_decay)],'weight_decay': args.weight_decay,'lr': args.learning_rate * 10 if args.cls_learning_rate == 0 else args.cls_learning_rate},
            {'params': [p for n, p in model.fc2.named_parameters() if any(nd in n for nd in no_decay)], 'weight_decay': 0.0,'lr': args.learning_rate * 10 if args.cls_learning_rate == 0 else args.cls_learning_rate},
        ])
    if hasattr(model, "awl"):
        optimizer_grouped_parameters.extend([
            {'params': model.awl.parameters(), 'weight_decay': 0}
        ])
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            table_id, input_tok, input_tok_type, input_tok_pos, input_tok_mask, \
                ...
                column_entity_mask, column_header_mask, labels_mask, labels, histogram = batch
            ...
            outputs = model(input_tok, input_tok_type, input_tok_pos, input_tok_mask,\
                            input_ent_text, input_ent_text_length, input_ent_type, input_ent_mask, column_entity_mask, 
...
def evaluate(args, config, eval_dataset, model, prefix=""):
    ...
    eval_dataloader = WikiDataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.eval_batch_size, is_train=False)
...
def main():
    ...
    config = config_class.from_pretrained(hybrid_model_config, cache_dir=args.cache_dir if args.cache_dir else None)
    
    type_vocab = load_type_vocab(args.type_vocab)

    model_class = AsymmetricDoubleTower
    ...
    if args.do_train:
        lm_checkpoint = torch.load(args.hybrid_model_path)
